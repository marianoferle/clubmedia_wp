...
function openNav() {
    ...
    document.getElementById('tarifa').innerHTML = this.precio3;
    ...
    document.getElementById('login123').className = "login-popup-1-active";
}

function closeNav() {
  document.getElementById('myNav').style.width = '0%';
  document.getElementById('login123').className = "";
}
  ...
  if (this.prefijo.value == ""  || this.telefono.value.trim().includes(' ') || this.telefono.value.includes('-') || this.telefono.value.includes('+') || this.telefono.value.includes(',') || this.telefono.value.includes('.')){
    ...
    switch (this.operadora.value) {
      ...
      case '2': // Movistar
        ...
        //url = '';
        document.getElementById('login123').className = "login-popup-3-active";
      break;
      case '3': // Personal
        this.msisdn = '54' + this.prefijo.value.trim() + this.telefono.value.trim();
        url = url + this.operadora.value + '&msisdn=' + this.msisdn;
      ...
      success: function (data) {
        if(data.status == "OK"){
          if("suscripto" in data){
            ...
            document.getElementById('login123').className = "login-popup-2-active";
          }
        } else {
