...
class ValidacionNavegacion {
	...
	function bloqueo_sitio(){
		...
		if (!$this->habilitado){
			...
			wp_enqueue_style('valida_style_general', plugins_url() . '/valida_acceso/css/style.css');
			wp_enqueue_style('valida_style_especifico', plugins_url() . '/valida_acceso/css/'.$this->carpeta.'/style.css');
		...
		if (isset($_COOKIE['CookieWIFI'])){
			return true;
				...
				if ($_REQUEST['landing_portal'] == 'wifi') {
					setcookie('CookieWIFI', time() , time()+14400, '/');
					return true;
				} else {
					return false;
				}
	...
	public function habilita_cookie_suscripto(){
		return setcookie($this->nombre_portal, time() , time()+14400, '/');
	}
	...
	function actualiza_cookie(){
		...
		setcookie($this->cookie_name,++$cookie,time() + 14400,'/');
	}
	...
	function habilita_cookie_visitante(){
		setcookie($this->cookie_name,0,time() + 14400,'/');
	}
        ...
        <h1><?= esc_html(get_admin_page_title()); ?></h1>
							...
							<tr valign="top">
								...
								<td>
									<select id="nombre_servicio" name="valida_acceso_nombre_portal" onchange="actualizaServicio();">
											...
											<option value="ClubMediaPortal" <?php if (get_option('valida_acceso_nombre_portal') == "ClubMediaPortal") echo "selected"; ?>>ClubMediaPortal</option>
									</select>
								</td>
							</tr>
								...
								<td>
									<select id="nombre_servicio" name="valida_acceso_nombre_carpeta">
											...
											<option value="signoya" <?php if (get_option('valida_acceso_nombre_carpeta') == "signoya") echo "selected"; ?>>signoya</option>
											<option value="clubmedia" <?php if (get_option('valida_acceso_nombre_carpeta') == "clubmedia") echo "selected"; ?>>clubmedia</option>
									</select>
				...
				function actualizaServicio(){
					...
					switch (nombreServicio) {
						...
						break;
						case 'ClubMediaPortal':
							document.getElementById('opcionIdServicio').value = 165;
						break;
